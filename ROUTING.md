...
After selection, optional Worker-side enrichment may attach elevation (Valhalla `/height`), weather (Open-Meteo), and motor-vehicle traffic samples (TomTom Flow Segment Data). Deterministic **POC fit** component scores and factual category badges are computed server-side when experimental toggles are enabled. See `poc/SCORING_AND_ENRICHMENT.md` for weights (`poc-scoring-v1`), limits, and missing-data rules. Motor-traffic exposure is an FRC/free-flow proxy — not bicycle safety or verified volume.
