...
ADR-020 adds Phase 1 start/end generation on the same endpoint: route the direct Start → End baseline, then try deterministic seeded interior detours sized from that baseline. Requested endpoints are hard constraints. Point-to-point shortlisting uses geometry-overlap instead of the loop-midpoint heuristic. Ordered stops and return routing remain Phase 2.

After selection, optional Worker-side enrichment may attach elevation (Valhalla `/height`), weather (Open-Meteo), and motor-vehicle traffic samples (TomTom Flow Segment Data). Deterministic **POC fit** component scores and factual category badges are computed server-side when experimental toggles are enabled. See `poc/SCORING_AND_ENRICHMENT.md` for weights (`poc-scoring-v2`), limits, and missing-data rules. Motor-traffic exposure is an FRC/free-flow proxy — not bicycle safety or verified volume.
